...
(function(){
  ...
  let loadedKey=null,loadingPromise=null;
  async function ensureVotingStats(){
    ...
    const ids=Array.isArray(m.lineup)?m.lineup.map(String):[];const key=`${m.id}|${ids.join(',')}`;
    if(loadedKey===key)return;if(loadingPromise)return loadingPromise;
    loadingPromise=(async()=>{try{const next={};const refs=ids.map(id=>db.collection('matches').doc(m.id).collection('stats').doc(id));const snaps=await Promise.all(refs.map(ref=>ref.get()));snaps.forEach((snap,i)=>{if(snap.exists)next[ids[i]]={id:ids[i],...(snap.data()||{})};});window.currentMatchStats=next;loadedKey=key;if(typeof populateVotes==='function')await populateVotes();}catch(e){console.error('BF Fix caricamento statistiche votazione:',e);}finally{loadingPromise=null;}})();
    return loadingPromise;
  }
  window.bfFixRefreshVoting=async function(){if(isPlayerRole())await ensureVotingStats();};
  function resetIfMatchChanged(){const m=activeMatch();const key=m?`${m.id}|${(m.lineup||[]).join(',')}`:'';if(key&&loadedKey&&key!==loadedKey)loadedKey=null;if(!key)loadedKey=null;}
  setTimeout(()=>{if(isAdminRole())window.bfFixLoadPendingRegistrations();},700);
  setInterval(async()=>{try{resetIfMatchChanged();if(isAdminRole())await window.bfFixLoadPendingRegistrations();const card=document.querySelector('#votingCard');if(card&&!card.classList.contains('hidden'))await ensureVotingStats();}catch(e){console.error('BF Fix periodic refresh:',e);}},1500);
})();
