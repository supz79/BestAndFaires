...
function applyRolePermissions(userData){
  ...
  if(adminButton) adminButton.style.display=admin?'':'none';
  if(!admin && document.querySelector('#admin')?.classList.contains('active')) show('dashboard');
  renderMatch();
}
...
function renderMatch(){
 const box=$("#rosterList"); if(!box)return; box.innerHTML="";
 const admin=isAdmin();
 players.forEach(p=>{
   const checked=state.lineup.includes(p.id);
   box.insertAdjacentHTML("beforeend",`<label class="check ${state.locked||!admin?"locked":""}"><input type="checkbox" data-player="${p.id}" ${checked?"checked":""} ${(state.locked||!admin)?"disabled":""}><span>${p.name}</span>${p.id===state.currentUser?'<small class="sub">Tu</small>':''}</label>`);
 });
 const lockBtn=$("#lockBtn");
 lockBtn.textContent=state.locked?"🔓 Distinta bloccata":"🔒 Blocca distinta";
 lockBtn.disabled=state.locked||!admin;
 lockBtn.style.display=admin?'':'none';
 if(state.locked){$("#votingCard").classList.remove("hidden");populateVotes();}
...
$("#rosterList").addEventListener("change",e=>{
 ...
 if(e.target.checked&&!state.lineup.includes(id))state.lineup.push(id);
 if(!e.target.checked)state.lineup=state.lineup.filter(x=>x!==id);
 save();
});
$("#lockBtn").onclick=()=>{
 if(!isAdmin())return;
 if(state.lineup.length<4){alert("Servono almeno 4 giocatori in distinta per votare i migliori 3.");return}
 state.locked=true;save();renderMatch();
};
...
function renderRanking(){const rows=calculate();$("#rankingTable").innerHTML=rows.map((p,i)=>`<div class="rank"><span class="pos">${i<3?["🥇","🥈","🥉"][i]:i+1}</span><div><b>${p.name}</b><span class="sub">${p.first}× 1° · ${p.second}× 2° · ${p.third}× 3°</span></div><span class="points">${p.points} pt</span></div>`).join("")||"<p class='muted'>Nessun risultato.</p>";}
...
$("#resetBtn").onclick=()=>{if(!isAdmin())return;localStorage.removeItem("bf_beta1");location.reload()};
renderMatch();
